...
(deftest test-transactions
  (testing "fetch"
    (let [users-bucket (atom {})
      ...
      (let [response-user (app (-> (mock/request :post "/v1/users") (mock/json-body {:name "John Doe"})))]
        (let [response-fetch (app (-> (mock/request :get (str (get-in response-user [:headers "Location"]) "/transactions"))))]
